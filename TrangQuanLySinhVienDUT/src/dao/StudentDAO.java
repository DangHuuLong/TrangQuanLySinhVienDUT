...
public class StudentDAO {
    ...
    public void insertBasic(Student s) {
        try (Connection conn = DBConnection.getConnection()) {
            ...
            // Insert vào users
            String userInsert = "INSERT INTO users (id, username, password, role) VALUES (?, ?, ?, ?)";
            try (PreparedStatement uStmt = conn.prepareStatement(userInsert)) {
                uStmt.setInt(1, s.getId());
                uStmt.setString(2, s.getUsername());
                uStmt.setString(3, s.getPassword());
                uStmt.setString(4, s.getRole());
                uStmt.executeUpdate();
            }
    ...
    public void updateField(int id, String field, Object value) {
